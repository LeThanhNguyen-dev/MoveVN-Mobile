...
export default function RegistrationScreen({ navigation, route }: Props) {
  ...
  return <AuthScreenLayout><View style={styles.stack}>
    ...
    {!isOwner ? <View style={styles.policyLinks}><Pressable onPress={() => navigation.navigate("Policies", { slug: "privacy-policy" })}><Text style={styles.link}>Chính sách bảo mật</Text></Pressable><Text style={styles.policyDot}>•</Text><Pressable onPress={() => navigation.navigate("Policies", { slug: "terms-of-service" })}><Text style={styles.link}>Điều khoản sử dụng</Text></Pressable></View> : null}
