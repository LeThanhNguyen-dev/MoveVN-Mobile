...
export default function ForgotPasswordScreen({ navigation }: Props) {
  ...
  return <AuthScreenLayout><View style={styles.stack}><AuthScreenHeader description="Nhập email để nhận mã OTP đặt lại mật khẩu." onBack={() => navigation.goBack()} title="Quên mật khẩu" />
    {error ? <AuthNotice error message={error} /> : null}<AuthField editable={!busy} icon={<Mail color="#746F7E" size={19} />} keyboardType="email-address" label="Email" onChangeText={setEmail} placeholder="ban@email.com" value={email} />
