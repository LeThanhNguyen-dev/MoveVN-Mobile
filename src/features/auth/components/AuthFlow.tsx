  ...
  return <SafeAreaView style={styles.safe}>
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.fill}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        <AuthLayout>
          {starting ? <View style={styles.center}><ActivityIndicator color="#6B19FF" size="large" /><Text style={styles.muted}>Đang kiểm tra phiên đăng nhập...</Text></View>
            : startupError || storageError ? <View style={styles.stack}>
              <AuthNotice error message={storageError || startupError} />
              <AuthButton busy={busy} icon={<RotateCcw size={18} color="#FFFFFF" />} onPress={() => { void run(async () => { if (storageError) await clearSession(); await start(); }); }} title="Thử lại" />
            ...
            : authenticated ? <View style={[styles.stack, styles.success]}>
              <View style={styles.successIcon}><Check size={30} color="#FFFFFF" /></View><Text style={styles.title}>Đăng nhập thành công</Text><Text style={[styles.muted, styles.centeredText]}>Chào mừng bạn trở lại với MoveVN.</Text>
              {error ? <AuthNotice error message={error} /> : null}<AuthButton busy={busy} icon={<LogOut size={18} color="#FFFFFF" />} onPress={() => { void run(signOut); }} title="Đăng xuất" />
            </View>
            : <View style={styles.stack}>
              {screen !== "login" ? <Pressable accessibilityLabel="Quay lại đăng nhập" accessibilityRole="button" disabled={busy} onPress={() => navigate("login")} style={styles.back}><ArrowLeft color="#6B19FF" size={21} /></Pressable> : null}
              ...
              {registration ? field("fullName", "Họ và tên", <UserRound size={19} color="#746F7E" />, { autoComplete: "name", maxLength: 200, placeholder: "Nguyễn Văn A" }) : null}
              {field("email", "Email", <Mail size={19} color="#746F7E" />, { autoComplete: "email", keyboardType: "email-address", maxLength: 256, placeholder: "ban@email.com" })}
              {registration ? field("phone", "Số điện thoại", <Phone size={19} color="#746F7E" />, { autoComplete: "tel", keyboardType: "phone-pad", maxLength: 10, placeholder: "0912345678" }) : null}
              {screen === "verify" || screen === "reset" ? <OtpField disabled={busy} error={errors.otp} onChange={(value) => update("otp", value)} value={fields.otp} /> : null}
              {showPassword ? field("password", screen === "reset" ? "Mật khẩu mới" : "Mật khẩu", <LockKeyhole size={19} color="#746F7E" />, { autoComplete: screen === "login" ? "current-password" : "new-password", password: true, placeholder: "Nhập mật khẩu" }) : null}
              {registration || screen === "reset" ? field("confirmPassword", screen === "reset" ? "Xác nhận mật khẩu mới" : "Xác nhận mật khẩu", <ShieldCheck size={19} color="#746F7E" />, { autoComplete: "new-password", password: true, placeholder: "Nhập lại mật khẩu" }) : null}
...
const styles = StyleSheet.create({
  safe: { flex: 1, backgroundColor: "#FAF6FF" }, fill: { flex: 1 }, scroll: { flexGrow: 1 }, stack: { gap: 16 }, heading: { gap: 7, marginBottom: 3 }, title: { color: "#101936", fontSize: 30, lineHeight: 38, fontWeight: "800" }, muted: { color: "#746F7E", fontSize: 15, lineHeight: 23, fontWeight: "500" }, centeredText: { textAlign: "center" }, center: { gap: 18, alignItems: "center", paddingVertical: 54 },
  back: { width: 42, height: 42, borderRadius: 21, borderWidth: 1, borderColor: "#E2D8F0", backgroundColor: "#FFFFFF", alignItems: "center", justifyContent: "center", marginBottom: 2 }, loginOptions: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 12, marginTop: -2 }, remember: { flexDirection: "row", alignItems: "center", gap: 8 }, checkbox: { width: 19, height: 19, borderWidth: 1, borderColor: "#CFC5DD", borderRadius: 5, backgroundColor: "#FFFFFF", alignItems: "center", justifyContent: "center" }, checkboxChecked: { borderColor: "#6B19FF", backgroundColor: "#6B19FF" }, rememberText: { color: "#625B6B", fontSize: 13, fontWeight: "600" }, link: { color: "#6B19FF", fontSize: 14, lineHeight: 21, fontWeight: "700" }, mutedLink: { color: "#746F7E", fontSize: 14, lineHeight: 21, fontWeight: "600" },
  termsBlock: { gap: 8 }, terms: { flexDirection: "row", alignItems: "flex-start", gap: 9 }, termsCheckbox: { marginTop: 1, flexShrink: 0 }, termsText: { flex: 1, color: "#625B6B", fontSize: 13, lineHeight: 20, fontWeight: "500" }, policyLinks: { flexDirection: "row", alignItems: "center", gap: 8, marginLeft: 28 }, policyDot: { color: "#9A91A6", fontSize: 12 }, error: { color: "#B4235A", fontSize: 13, lineHeight: 19, fontWeight: "500" },
  googleButton: { minHeight: 52, borderWidth: 1, borderColor: "#DDD4EA", borderRadius: 12, backgroundColor: "#FFFFFF", flexDirection: "row", justifyContent: "center", alignItems: "center", gap: 10 }, googleMark: { color: "#4285F4", fontSize: 21, lineHeight: 25, fontWeight: "800" }, googleText: { color: "#343042", fontSize: 15, fontWeight: "700" }, divider: { flexDirection: "row", alignItems: "center", gap: 12, marginVertical: 2 }, dividerLine: { height: 1, flex: 1, backgroundColor: "#E4DDEA" }, dividerText: { color: "#91899B", fontSize: 13, fontWeight: "600" }, bottomText: { color: "#625B6B", textAlign: "center", fontSize: 14, lineHeight: 22, fontWeight: "500" }, textAction: { alignItems: "center", paddingVertical: 2 }, dim: { opacity: 0.52 }, success: { alignItems: "center", paddingVertical: 24 }, successIcon: { width: 60, height: 60, borderRadius: 30, backgroundColor: "#6B19FF", alignItems: "center", justifyContent: "center", marginBottom: 4 },
});
