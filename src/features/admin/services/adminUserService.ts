...
import { appendUploadFile, isUploadFileInput, type UploadFileInput } from "@/types/upload";
import type {
  AdminUserListParams,
  AdminUserListItem,
  AdminUserDetail,
...
export async function createOwner(request: CreateOwnerRequest) {
  ...
  Object.entries(request).forEach(([key, value]) => {
    ...
    if (isUploadFileInput(value)) {
      appendUploadFile(formData, key, value);
      return;
    }
...
export async function previewOwnerOcr(
  fullName: string,
  nationalIdFrontImage?: UploadFileInput | null,
  driverLicenseFrontImage?: UploadFileInput | null,
) {
  const formData = new FormData();
  formData.append("fullName", fullName);
  if (nationalIdFrontImage) appendUploadFile(formData, "nationalIdFrontImage", nationalIdFrontImage);
  if (driverLicenseFrontImage) appendUploadFile(formData, "driverLicenseFrontImage", driverLicenseFrontImage);
